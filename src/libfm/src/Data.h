...
class DataMetaInfo {
	public:
		...
		void loadGroupsFromFile(string filename) {
			assert(fileexists(filename));
			attr_group.load(filename);
			num_attr_groups = 0;
			for (uint i = 0; i < attr_group.dim; i++) {
				num_attr_groups = max(num_attr_groups, attr_group(i)+1);
			}
			num_attr_per_group.setSize(num_attr_groups);
		...
		void debug() {
			cout << "#attr=" << attr_group.dim << "\t#groups=" << num_attr_groups << endl;
			for (uint g = 0; g < num_attr_groups; g++) {
				cout << "#attr_in_group[" << g << "]=" << num_attr_per_group(g) << endl;
			}
		}
};
...
class Data {
	protected:
		...
		bool has_x;
	public:	
		Data(uint64 in_cache_size, bool in_has_x, bool in_has_xt):cache_size(in_cache_size),
		has_x(in_has_x), has_xt(in_has_xt){}

		LargeSparseMatrix<DATA_FLOAT>* data_t = NULL;
		LargeSparseMatrix<DATA_FLOAT>* data = NULL;
...
void Data::load(string filename) {
	...
	cout << "has x = " << has_x << endl;
	cout << "has xt = " << has_xt << endl;
	assert(has_x || has_xt);
	...
	if (load_from > 0) {
		...
		min_target = +numeric_limits<DATA_FLOAT>::max();
		max_target = -numeric_limits<DATA_FLOAT>::max();
		for (uint i = 0; i < this->target.dim; i++) {
			min_target = min(this->target(i), min_target);
			max_target = max(this->target(i), max_target);				
		}
		num_cases = target.dim;

		cout << "num_cases=" << this->num_cases << "\tnum_values=" << num_values << "\tnum_features=" << this->num_feature << "\tmin_target=" << min_target << "\tmax_target=" << max_target << endl;
		return;
	}
...
void Data::debug() {
	if (has_x) {
		for (data->begin(); (!data->end()) && (data->getRowIndex() < 4); data->next() ) {
			cout << target(data->getRowIndex());
			for (uint j = 0; j < data->getRow().size; j++) {
				cout << " " << data->getRow().data[j].id << ":" << data->getRow().data[j].value;	
			}
			cout << endl;
		}
	}	
