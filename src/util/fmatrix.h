...
template <typename T> class LargeSparseMatrix {
	public:
		...
		void saveToBinaryFile(string filename) {
			cout << "printing to " << filename << endl; cout.flush();
...
template <typename T> class LargeSparseMatrixHD : public LargeSparseMatrix<T> {
		...
		void readcache() {
			...
			do {
				...
				if ((this_row.size + number_of_valid_entries_in_cache) > cache.dim) {
					in.seekg(- (long int) sizeof(uint), ios::cur);
					break;
				}
	...
	public:
		LargeSparseMatrixHD(string filename, uint64 cache_size) { 
			...
			if (cache_size == 0) {
				cache_size = numeric_limits<uint64>::max();
			}
			...
			uint num_rows_in_cache;
			{
				...
				if (dummy > static_cast<uint64>(numeric_limits<uint>::max())) {
					num_rows_in_cache = numeric_limits<uint>::max();
				} else {
			...
			cout << "num entries in cache=" << num_entries_in_cache << "\tnum rows in cache=" << num_rows_in_cache << endl;
...
//		~LargeSparseMatrixHD() { in.close(); }
		...
		uint getNumRows() { return num_rows; };
		uint getNumCols() { return num_cols; };
		uint64 getNumValues() { return num_values; };
		...
		void begin() {
			...
			number_of_valid_rows_in_cache = 0;
			number_of_valid_entries_in_cache = 0;
			in.seekg(sizeof(file_header), ios_base::beg);
			readcache();
		}

		bool end() { return row_index >= num_rows; }

		sparse_row<T>& getRow() { return data(position_in_data_cache); }
		uint getRowIndex() { return row_index; }
...
template <typename T> class LargeSparseMatrixMemory : public LargeSparseMatrix<T> {
	...
	public:
		...
		uint64 num_values;
		void begin() { index = 0; };
		bool end() { return index >= data.dim; }
		void next() { index++;}
		sparse_row<T>& getRow() { return data(index); };
		uint getRowIndex() { return index; };
		uint getNumRows() { return data.dim; };
		uint getNumCols() { return num_cols; };
		uint64 getNumValues() { return num_values; };
